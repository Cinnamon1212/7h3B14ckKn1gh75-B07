...
class maths(commands.Cog):
    ...
    async def calculator(self, ctx, *, query=None):
        text = f"""
Usage: {discord.PREFIX}calc [calculation]
...
[+(n)] Positive numbers
    ...
    async def std_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            text = f"""
Usage: {discord.PREFIX}std [List of numbers]
Please split numbers with ','
    ...
    async def round_(self, ctx, decimalplaces=None, *, numbers=None):
        text = f"""
        Usage: {discord.PREFIX}rnd [Decimal places] [numbers]
    ...
    async def ceil_(self, ctx, *, numbers=None):
        if numbers is None:
            text = f"""
Usage: {discord.PREFIX}ceil [list of values]
            """
            await ctx.send(f"```{text}```")
        else:
            numbers = findnumbers(numbers)
    ...
    async def floor_(self, ctx, *, numbers=None):
        if numbers is None:
            text = f"Usage: {discord.PREFIX}floor [list of values]"
            await ctx.send(f"```{text}```")
        else:
            numbers = findnumbers(numbers)
            floored = []
    ...
    async def gcd_(self, ctx, *, numbers=None):
        text = f"Usage: {discord.PREFIX}gcd [num1] [num2]"
        if numbers is None:
            await ctx.send(f"```{text}```")
        else:
            numbers = findnumbers(numbers)
    ...
    async def quadratic(self, ctx, a=None, b=None, c=None):
        text = f"Usage: {discord.PREFIX}quadratic [a] [b] [c]"
