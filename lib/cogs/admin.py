...
class admin(commands.Cog):
    ...
    async def kick(self, ctx, member: discord.Member = None, *, reason="Unspecified reason"):
        text = f"Usage: {discord.PREFIX}kick [member] (reason)"
        if member is None:
    ...
    async def ban(self, ctx, member: discord.Member = None, *, reason="Unspecified reason"):
        text = f"Usage: {discord.PREFIX}ban [member] (reason)"
        if member is None:
            await ctx.send(f"```{text}```")
        else:
            bot_ = ctx.guild.get_member(self.client.user.id)
    ...
    async def unban(self, ctx, *, member):
        text = f"Usage: {discord.PREFIX}unban [member/ID]"
        if len(member) == 18:
            try:
                member = int(member)
    ...
    async def clear(self, ctx, amount=1, channel_id=None):
        text = f"""Usage: {discord.PREFIX}clear [Amount/ID] (channel)
...
if the command is not in the channel, please specify the channel"""
    ...
    async def userinfo(self, ctx, *, member: discord.Member=None):
        text = f"Usage: {discord.PREFIX}userinfo [@member]"
        if member == None:
            member = ctx.author
        try:
...
Please ensure this is ran within the context of a server/guild\nUsage: {discord.PREFIX}serverinfo```""")
    ...
    async def rolemod(self, ctx, method, member: discord.Member, role: discord.Role):
        text = f"Usage: {discord.PREFIX}rolemod [add/remove] [member] [role]"
        if method.lower() == "add":
    ...
    async def rolemod_error(self, ctx, error):
        text = f"Usage: {discord.PREFIX}rolemod [method] [member] [role]"
        if isinstance(error, commands.MissingPermissions):
